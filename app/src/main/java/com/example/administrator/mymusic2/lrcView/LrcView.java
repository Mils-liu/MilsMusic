...
import static com.example.administrator.mymusic2.lrcView.LrcViewState.Seeking;
import static com.example.administrator.mymusic2.lrcView.LrcViewState.normal;
...
public class LrcView extends View implements ILrcView {
            ...
            case MotionEvent.ACTION_UP:
                if (mRows == null || mRows.size() == 0) {
                    if (mLrcViewListener != null) {
                        mLrcViewListener.onClick();
                    }
                    Log.d("LrcViewTAG","onTouch");
                    DoClick();
                    return true;
                }
    ...
    private void seekToPosition() {
        ...
        postInvalidate();
        if (mLrcViewListener != null) {
            mLrcViewListener.onSeek(mRows.get(HeightLightRowPosition), mRows.get(HeightLightRowPosition).CurrentRowTime);
        }
    }
    ...
    private void drawNormalRow(int rawIndex, LrcRow lrcRow, Canvas canvas, float rowX) {
        ...
        for (LrcShowRow sr : showRows) {
            ...
            Log.d("LrcViewTAG","歌词:"+sr.Data);
            Log.d("LrcViewTAG","sr.YPosition:"+sr.YPosition);
